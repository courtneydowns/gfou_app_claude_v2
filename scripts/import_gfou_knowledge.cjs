...
const DO_NOT_PATTERNS = [
  /^\s*(?:[—\-]\s*|\d+\.\s*)(Do not\s.{6,})/i,
  /^\s*(?:[—\-]\s*|\d+\.\s*)(Never\s.{6,})/i,
  /^\s*DO NOT\s*[:—\-]?\s*(.{6,})/,
];
...
/** Extract explicit negative directives from a text string. */
function extractDoNotDoThisFromText(text) {
  const candidates = [];
  const seen = new Set();
  for (const line of String(text || "").split("\n")) {
    for (const pat of DO_NOT_PATTERNS) {
...
function normalizeScene(oldScene, number) {
  ...
  const startHereCandidates = [
    oldScene.startHere, oldScene.start_here, oldScene.startLines,
    oldScene.entryPrompts, oldScene.openingPrompts,
  ];
  let start_here = [];
  for (const c of startHereCandidates) {
    const parsed = safeJson(c, c);
    if (Array.isArray(parsed)) { start_here = parsed.map(String).filter(Boolean); break; }
  }

  const stuckOptionsCandidates = [
    oldScene.stuckOptions, oldScene.stuck_options,
    oldScene.bodyOptions, oldScene.nextMoves,
  ];
  let stuck_options = [];
  for (const c of stuckOptionsCandidates) {
    const parsed = safeJson(c, c);
    if (Array.isArray(parsed)) { stuck_options = parsed.map(String).filter(Boolean); break; }
  }

  const successCandidates = [
    oldScene.successLooksLike, oldScene.success_looks_like,
    oldScene.successCriteria, oldScene.doneWhen,
  ];
  let success_looks_like = [];
  for (const c of successCandidates) {
    const parsed = safeJson(c, c);
    if (Array.isArray(parsed)) { success_looks_like = parsed.map(String).filter(Boolean); break; }
  }

  const rule = pickText(
    oldScene.rule, oldScene.nonNegotiableRule, oldScene.non_negotiable_rule,
    oldScene.hardRule, oldScene.constraint
  );
  const stuck_directive = pickText(
    oldScene.stuckDirective, oldScene.stuck_directive,
    oldScene.forwardDirective, oldScene.directive
  );

  const notesParts = [];
  for (const key of ["summary", "notes", "arcFunction", "function",
                      "sourceId", "chapter", "beat", "pressure"]) {
    if (oldScene[key]) notesParts.push(`${key}: ${String(oldScene[key])}`);
  }

  // New structured fields
  const source_anchor  = (id !== title) ? `${id} — ${title}` : id;
  const dont_forget    = stuck_directive || (stuck_options.length > 0 ? stuck_options[0] : "");
  const do_not_do_this = extractDoNotDoThisFromText(rule);

  const stuck_bank = { ...EMPTY_STUCK_BANK };
  if (stuck_options.length > 0) {
  ...
  return {
    ...
    source_anchor,
    dont_forget,
    do_not_do_this:    JSON.stringify(do_not_do_this),
    stuck_bank:        JSON.stringify(stuck_bank),
  };
}

function extractDraft(oldScene) {
  return pickText(
    oldScene.draft, oldScene.text, oldScene.content, oldScene.currentDraft,
    oldScene.manuscriptText, oldScene.body, oldScene.v0Draft
  );
}
...
function main() {
  ...
  const tx = db.transaction(() => {
    ...
    for (const item of KNOWLEDGE_FILES) {
      ...
      upsertSource.run(id, item.title, content, JSON.stringify(item.tags), null, "global");
      if (item.alsoRule) {
        upsertRule.run(
          `rule:${item.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")}`,
          item.title, content, "global", null
        );
      }
    }
    ...
    if (chapterBlueprint) {
      upsertSource.run(
        ...
        chapterBlueprint, JSON.stringify(["chapter-blueprint", "autosave", "structure"]),
        null, "global"
      );
    }
    ...
    if (sourceOfTruth) {
      upsertSource.run(
        ...
        sourceOfTruth, JSON.stringify(["source-of-truth", "autosave"]),
        null, "global"
      );
    }
    ...
    if (oldContinuityLedger) {
      upsertSource.run(
        ...
        oldContinuityLedger, JSON.stringify(["continuity", "ledger", "autosave"]),
        null, "global"
      );
    }
  });
  ...
  const counts = {
    ...
    has_source_anchor: db.prepare("SELECT COUNT(*) AS n FROM scenes WHERE source_anchor IS NOT NULL AND TRIM(source_anchor) != ''").get().n,
    has_dont_forget:   db.prepare("SELECT COUNT(*) AS n FROM scenes WHERE dont_forget IS NOT NULL AND TRIM(dont_forget) != ''").get().n,
    has_stuck_bank:    db.prepare("SELECT COUNT(*) AS n FROM scenes WHERE stuck_bank IS NOT NULL AND stuck_bank NOT IN ('{}','') AND json_array_length(json_extract(stuck_bank,'$.use_a_line')) > 0").get().n,
  };
