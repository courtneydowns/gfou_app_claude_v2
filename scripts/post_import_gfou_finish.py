...
def active_where(scene_cols):
    if "archived"   in scene_cols: return "(COALESCE(archived, 0)=0 OR archived IS NULL)"
    if "is_archived" in scene_cols: return "(COALESCE(is_archived, 0)=0 OR is_archived IS NULL)"
    if "isArchived"  in scene_cols: return "(COALESCE(isArchived, 0)=0 OR isArchived IS NULL)"
    if "status"      in scene_cols: return "(status IS NULL OR LOWER(status) NOT IN ('archived','hidden','smoke-test','smoke_test'))"
    return "1=1"
...
def fallback_stuck_options(scene):
    """
    Returns scene-type-appropriate guidance lines.
    These are based on the scene's inferred domain — NOT generic placeholders.
    """
...
def main():
    ...
    bank_col     = first_col(scene_cols, ["stuck_bank", "stuckBank"])
    ...
    if not id_col or not stuck_col:
        ...
        for scene in scenes:
            opts_existing  = split_options(scene.get(stuck_col))
            bank_existing  = parse_stuck_bank(scene.get(bank_col) if bank_col else None)
            bank_populated = stuck_bank_has_content(bank_existing)

            # Fill stuck_options if empty
            if not opts_existing:
                ...
                filled_stuck += 1
                # Re-read for bank fill below
                opts_existing = opts

            # Fill stuck_bank.use_a_line if bank is empty and we have options
            if bank_col and not bank_populated and opts_existing:
                new_bank = EMPTY_STUCK_BANK.copy()
                new_bank["use_a_line"] = opts_existing
                bank_payload = json.dumps(new_bank, ensure_ascii=False)
                if updated_col:
                    cur.execute(
                        f"UPDATE scenes SET {bank_col}=?, {updated_col}=? WHERE {id_col}=?",
                        (bank_payload, now(), scene[id_col]),
                    )
                else:
                    cur.execute(
                        f"UPDATE scenes SET {bank_col}=? WHERE {id_col}=?",
                        (bank_payload, scene[id_col]),
                    )
                filled_bank += 1
    ...
    if not table_exists(cur, "continuity_ledger"):
        ...
        if not content_col:
            ...
            for row in rows:
                ...
                insert_cols = []
                vals        = []
                if title_col:    insert_cols.append(title_col);    vals.append(row["title"])
                insert_cols.append(content_col);                   vals.append(row["content"])
                if category_col: insert_cols.append(category_col); vals.append(row["category"])
                if source_col:   insert_cols.append(source_col);   vals.append(row["source"])
                if status_col:   insert_cols.append(status_col);   vals.append(row["status"])
                if created_col:  insert_cols.append(created_col);  vals.append(row["created_at"])
                if updated_col2: insert_cols.append(updated_col2); vals.append(row["updated_at"])
    ...
    if not table_exists(cur, "revision_queue"):
        ...
        if rev_count > 0:
            ...
            tasks = [
                {"title": "Phase 2 continuity pass",     "content": "After 3–5 V0 scenes are drafted, check continuity against imported ledger and source material.", "priority": "medium"},
                {"title": "Voice consistency pass",       "content": "Compare drafted scenes against GFOU Voice Bible and Protection Rules before polishing.",          "priority": "medium"},
                {"title": "Scene completion audit",       "content": "Verify each scene has physical progression, no explanation leakage, and a clear stop point.",    "priority": "medium"},
            ]
            ...
            if not content_col and not title_col:
                ...
                for task in tasks:
                    insert_cols = []
                    vals        = []
                    if title_col:    insert_cols.append(title_col);    vals.append(task["title"])
                    if scene_col:    insert_cols.append(scene_col);    vals.append("GLOBAL")
                    if content_col:  insert_cols.append(content_col);  vals.append(task["content"])
                    if status_col:   insert_cols.append(status_col);   vals.append("open")
                    if priority_col: insert_cols.append(priority_col); vals.append(task["priority"])
                    if created_col:  insert_cols.append(created_col);  vals.append(now())
                    if updated_col3: insert_cols.append(updated_col3); vals.append(now())
                    placeholders = ",".join(["?"] * len(insert_cols))
                    cur.execute(
    ...
    if stuck_col:
        cur.execute(f"""
            ...
            WHERE {where_active}
              AND {stuck_col} IS NOT NULL AND TRIM({stuck_col}) != ''
        """)
    ...
    if bank_col:
        cur.execute(f"""
            SELECT COUNT(*) FROM scenes
            WHERE {where_active}
              AND {bank_col} IS NOT NULL
              AND {bank_col} NOT IN ('{{}}','')
              AND json_array_length(json_extract({bank_col},'$.use_a_line')) > 0
        """)
        bank_populated_count = cur.fetchone()[0]
    else:
        bank_populated_count = "unknown"
    ...
    print(f"stuck_bank.use_a_line filled: {filled_bank}")
    ...
    print(f"  active scenes with stuck_bank.use_a_line: {bank_populated_count} of {active_scenes}")
